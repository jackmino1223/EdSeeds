...
public class AutoCompleteTextField:UITextField {
    ...
    public var maximumAutoCompleteCount = 50
...
//        let screenSize = UIScreen.main.bounds.size
        let tableView = UITableView(frame: CGRect(x: self.frame.origin.x,
                                                  y: self.frame.origin.y + self.frame.height,
                                                  width: self.frame.width, // - (self.frame.origin.x * 2),
                                                  height: 30.0))
    ...
    private func reload(){
        if enableAttributedText{
            let attrs : [String: Any] = [NSForegroundColorAttributeName:autoCompleteTextColor,
    ...
    public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if autoCompleteStrings != nil {
            if autoCompleteStrings!.count < maximumAutoCompleteCount {
                cellCount = autoCompleteStrings!.count
            } else {
                cellCount = maximumAutoCompleteCount
            }
        }
        return cellCount // autoCompleteStrings != nil ? (autoCompleteStrings!.count > maximumAutoCompleteCount ? maximumAutoCompleteCount : autoCompleteStrings!.count) : 0
    }
